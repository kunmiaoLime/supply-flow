...
directory, structured context analysis, source documents, and source-reader
instructions.

1. Read this workflow and the supplied RFC Markdown template before reading
   any source.
2. Read both structured context-analysis files supplied in the initial goal
   when they exist. Treat them as the source of truth for the template's
   Review Notes Gaps and Conflicts subsections. Do not modify either file. If
   either file is unavailable or invalid, record that fact in the relevant
   Review Notes subsection and do not invent entries.
3. For each configured document, read its listed reader instructions first,
...
4. Inspect only the repository scopes listed in the initial goal. Determine whether
...
5. Synthesize the available requirements into the template's structure.
...
6. In Review Notes, copy every current structured context gap and conflict into
   its corresponding subsection, preserving all required details from the
   template. Do not omit entries because they are low severity or unrelated to
   the selected repository scope.
7. For every API endpoint relevant to the selected repository scope, complete
...
8. Complete the template's Implementation Plan with a module-by-module
...
9. Complete the Feature Flags section with every runtime flag or configuration
   needed to stage, target, measure, or roll back the change. When no feature
   flag is needed, state `Not required` and explain why.
10. Add every source that materially informed the draft to Related Product
   Documents or Appendix. Include links and concise labels.
11. Choose a descriptive kebab-case filename based on the RFC title. Create
   the draft as `<DRAFT_DIRECTORY>/<filename>.md`. Do not overwrite an
   existing RFC draft; add a numeric suffix when needed.
12. Run the exact draft-registration command supplied in the initial goal,
   replacing `<RFC_DRAFT_LINK>` with the new project-relative path. This is
   the only allowed project-metadata change.
13. Do not modify repository code, project context, source documents, or
   existing RFC drafts. Do not create a Confluence page in this session.
14. After writing the draft, verify the file exists, summarize the important
